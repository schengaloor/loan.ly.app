...
class LoanlyWebAppSupabaseUser extends BaseAuthUser {
  LoanlyWebAppSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> loanlyWebAppSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = LoanlyWebAppSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
